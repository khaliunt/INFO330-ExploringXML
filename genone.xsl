...
<xsl:apply-templates select="pokemon[pokedexNumber <= 151 and generation = '1']" />
</xsl:template>

<xsl:template match="pokemon">
<xsl:value-of select="name" /> (<xsl:value-of select="pokedexNumber" />): <xsl:value-of select="type" /> |
</xsl:template>
