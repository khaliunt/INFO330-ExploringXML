...
Strong attackers: <xsl:value-of select="count(pokemon[stats/attack > 150])" />
<xsl:apply-templates select="pokemon[stats/attack > 150]" />
Strong defenders: <xsl:value-of select="count(pokemon[stats/defense > 150])" />
<xsl:apply-templates select="pokemon[stats/defense > 150]" />
Fast: <xsl:value-of select="count(pokemon[stats/speed > 150])" />
<xsl:apply-templates select="pokemon[stats/speed > 150]" />
</xsl:template>
...
<xsl:template match="/pokedex">
  <html>
  <body>
  <h2>Strong Attackers: <xsl:value-of select="count(pokemon[stats/attack > 150])" /></h2>
  <table border="1">
    <tr bgcolor="#9acd32">
      <th>Name</th>
      <th>Type</th>
    ...
    <h2>Strong Defenders: <xsl:value-of select="count(pokemon[stats/defense > 150])" /></h2>
  <table border="1">
    <tr bgcolor="#9acd32">
      <th>Name</th>
      <th>Type</th>
    ...
    <h2>Fast: <xsl:value-of select="count(pokemon[stats/speed > 150])" /></h2>
  <table border="1">
    <tr bgcolor="#9acd32">
      <th>Name</th>
      <th>Type</th>
...
<xsl:template match="pokemon">
    <tr>
      ...
      <td><xsl:value-of select="concat('#', pokedexNumber)" /></td>
